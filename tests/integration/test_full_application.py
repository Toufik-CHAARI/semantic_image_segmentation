...
class TestFullApplication:
    ...
    def test_different_image_formats_workflow(self, client):
        ...
        for format_name in formats:
            ...
            with patch('app.api.segmentation.segmentation_service.segment_image') as mock_service:
                ...
                files = {"file": (f"test.{format_name.lower()}", img_bytes.getvalue(), f"image/{format_name.lower()}")}
                response = client.post("/api/segment", files=files)
    ...
    def test_application_configuration_workflow(self, client):
        ...
        for pattern in expected_patterns:
            assert any(pattern in listed_endpoint for listed_endpoint in data["endpoints"])
